...
sys_execv(const_userptr_t program, userptr_t *args)
{
    ...
    /* Step 1: Copy the program path and verify it */
    ...
    /* Step 2: Count arguments and calculate total size */
    if (args != NULL) {
        userptr_t *temp_args = args;
        while (1) {
            userptr_t arg;
            result = copyin((const_userptr_t)temp_args, &arg, sizeof(userptr_t));
            if (result) {
                goto err1;
            }
            if (arg == NULL) break;
        ...
        for (int i = 0; i < nargs; i++) {
            ...
            result = copyin((const_userptr_t)&args[i], &arg, sizeof(userptr_t));
            if (result) {
                goto err2;
            }
            
            result = copyinstr(arg, NULL, ARG_MAX, &len);
            if (result != EFAULT && result != 0) {
                goto err2;
            }
        ...
        for (int i = 0; i < nargs; i++) {
            ...
            kargs[i] = bufptr;
            result = copyinstr(arg, bufptr, ARG_MAX, &len);
            if (result) {
                goto err3;
            }
